...
exports.uploadUsers = catchAsyncErrors(async (req, res, next) => {
  // Start transaction
  ...
  try {
    ...
    for (const userData of studentData) {
      ...
      if (!teacher) {
        teacher = await teacherRepository.findByEmail(teacherEmail);
        if (!teacher) {
    ...
    return next(
      new ErrorHandler(error.message || "Error processing upload", 400)
    );
  }
});
...
exports.getMyStudents = catchAsyncErrors(async (req, res, next) => {
  ...
  const formattedStudents = students.map((student) => ({
    ...
    enrolledCourses: [], // You would populate this from StudentCourses table
  }));
...
exports.getStudentsByTeacherId = catchAsyncErrors(async (req, res, next) => {
  ...
  // Find all students associated with this teacher
  const students = await studentRepository.findByTeacherId(teacherId);
